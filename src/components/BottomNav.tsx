...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  if (isResident) {
    return (
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-200 h-[60px] flex items-center justify-between px-3 relative shadow-xs">
        <div className="w-full max-w-md mx-auto flex items-center justify-between h-full">
          ...
          >
            <LayoutGrid className={`w-5 h-5 ${activeTab === 'home' ? 'stroke-[2.2px] text-blue-600' : 'stroke-[1.8px]'}`} />
            <span className={`text-[10px] leading-tight mt-0.5 font-medium truncate max-w-full ${activeTab === 'home' ? 'font-bold text-blue-600' : 'text-slate-500'}`}>
              Dashboard
            </span>
          </button>
          ...
          >
            <div className="relative">
              <FolderKanban className={`w-5 h-5 ${activeTab === 'patients' || activeTab === 'review' ? 'stroke-[2.2px] text-blue-600' : 'stroke-[1.8px]'}`} />
              <span className="absolute -top-1.5 -right-2 bg-blue-600 text-white text-[9px] font-extrabold rounded-full h-4 min-w-[16px] px-1 flex items-center justify-center">
                1
              </span>
            </div>
            <span className={`text-[10px] leading-tight mt-0.5 font-medium truncate max-w-full ${activeTab === 'patients' || activeTab === 'review' ? 'font-bold text-blue-600' : 'text-slate-500'}`}>
              My Cases
            </span>
          </button>
          ...
          <div className="flex-1 flex justify-center items-center h-full">
            <button
              ...
              className="w-13 h-13 -mt-6 bg-blue-600 hover:bg-blue-700 text-white rounded-full flex items-center justify-center shadow-lg shadow-blue-500/40 border-4 border-slate-50 transition-transform active:scale-95 cursor-pointer z-20 shrink-0"
              title="Log Case"
              aria-label="Log Case"
            >
          ...
          >
            <MessageSquareText className={`w-5 h-5 ${activeTab === 'reports' ? 'stroke-[2.2px] text-blue-600' : 'stroke-[1.8px]'}`} />
            <span className={`text-[10px] leading-tight mt-0.5 font-medium truncate max-w-full ${activeTab === 'reports' ? 'font-bold text-blue-600' : 'text-slate-500'}`}>
              Remarks
            </span>
          </button>
          ...
          >
            <SettingsIcon className={`w-5 h-5 ${activeTab === 'settings' ? 'stroke-[2.2px] text-blue-600' : 'stroke-[1.8px]'}`} />
            <span className={`text-[10px] leading-tight mt-0.5 font-medium truncate max-w-full ${activeTab === 'settings' ? 'font-bold text-blue-600' : 'text-slate-500'}`}>
              Settings
            </span>
          </button>
        </div>
