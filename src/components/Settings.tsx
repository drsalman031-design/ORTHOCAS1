...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const [academicYear, setAcademicYear] = useState(profile.academicYear || 'Senior MDS / Faculty');
  const [supervisorName, setSupervisorName] = useState(profile.supervisorName || 'Dr. Sunita Patil');
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="space-y-0.5">
              <h2 className="text-base font-bold text-slate-900">{studentName || currentUser.name}</h2>
              <p className="text-[11px] font-semibold text-teal-700">
                {currentUser.designation || 'MDS, Orthodontics'} • {currentUser.role}
              </p>
            </div>
          </div>
