...
const CephFieldCard: React.FC<CephFieldCardProps> = ({
  ...
  if (inference) {
    if (inference.status === 'normal') {
      inputBorderClass = 'border-emerald-400 focus:ring-emerald-500 bg-emerald-50/20';
      badgeStyle = 'bg-[#E6F4EA] border-[#A8DAB5] text-emerald-800 font-semibold';
      Icon = CheckCircle2;
    } else if (inference.status === 'high') {
      inputBorderClass = 'border-red-400 focus:ring-red-500 bg-red-50/20';
      badgeStyle = 'bg-[#FCE8E6] border-[#F5C2C7] text-[#A50E0E] font-bold';
      Icon = AlertTriangle;
    } else if (inference.status === 'low') {
      inputBorderClass = 'border-blue-400 focus:ring-blue-500 bg-blue-50/20';
      badgeStyle = 'bg-[#E8F0FE] border-[#B6D0FE] text-blue-900 font-bold';
      Icon = AlertTriangle;
    } else if (inference.status === 'amber') {
      inputBorderClass = 'border-amber-400 focus:ring-amber-500 bg-amber-50/20';
      badgeStyle = 'bg-[#FEF7E0] border-[#FDE293] text-amber-900 font-bold';
      Icon = AlertTriangle;
    }
  }

  return (
    <div className="p-3 bg-white rounded-xl border border-slate-200/90 shadow-2xs space-y-2 flex flex-col justify-between hover:border-slate-300 transition-colors">
      {/* Top Row: Label Left, Touch-friendly Numeric Input Box Right */}
      ...
      {/* Middle Row: Norm Reference & Unit Badge */}
      <div className="flex items-center justify-between text-[10px] text-slate-500 font-medium pt-1 border-t border-slate-100">
        <span className="truncate">Norm: {normText}</span>
        {unit && (
          <span className="px-1.5 py-0.2 rounded bg-slate-100 text-slate-600 font-mono text-[9px] uppercase font-bold shrink-0 ml-1">
            {unit}
          </span>
        )}
      </div>

      {/* Bottom Row: Dynamic Inline Inference Chip / Badge */}
      {inference ? (
        <div className={`flex items-start gap-1.5 px-2.5 py-1.5 rounded-lg border text-[11px] leading-tight ${badgeStyle}`}>
          <Icon className="w-3.5 h-3.5 shrink-0 mt-0.5" />
          <span>{inference.text}</span>
        </div>
      ) : (
        <div className="text-[10px] italic text-slate-400 px-1 py-0.5">
          Enter value for inline inference
        </div>
      )}
    </div>
  );
...
export const ComprehensiveCephAnalysis: React.FC<ComprehensiveCephAnalysisProps> = ({
  ...
  // Derived dataset for active stage
  const activeData: StageData = useMemo(() => {
    return currentDataset[activeStage] || DEFAULT_STAGE_DATA;
  }, [activeStage, currentDataset]);
  ...
  return (
    <div className="w-full bg-white border border-slate-200/90 rounded-2xl shadow-xs overflow-hidden transition-all my-3">
      ...
      <div
        onClick={onToggle}
        className="w-full bg-white text-slate-900 p-3.5 sm:p-4 flex items-center justify-between cursor-pointer select-none hover:bg-slate-50 border-b border-slate-200 transition-colors"
      >
